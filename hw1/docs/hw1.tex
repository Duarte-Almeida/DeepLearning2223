...
\begin{document}
    ...
    \begin{questions}
        ...
        \begin{parts}
            ...
            % \begin{align*}
            %     \text{Final accuracy on validation set: } 0.7236 \\
            %     \text{Final accuracy on test set: } 0.6193
            % \end{align*}
            \begin{table}[h!]
            \centering
                \begin{tabular}{|l|l|}
            ...
            \part %1.1(b)
            For the logistic regression, the correspondent \texttt{update\_weights} method was also obtained, and using a learning rate of $\eta = 0.001$, the model was trained for 20 epochs, having the accuracies for the validation and test set also been plotted:
            \newpage
            %\vspace{-2.5em}
            \begin{figure}[h!]
                \centering
        ...
        \begin{parts}
            ...
            Nevertheless, an MLP that uses linear activation functions ($g(z) = z$) \textbf{does not display this feature}, it corresponds to a linear classifier, effectively functioning like a simple perceptron:
            \begin{align*}
    ...
    \begin{questions}
        \question
        The best configuration in terms of \textbf{validation accuracy} is the one with 
